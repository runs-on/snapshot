...
`force-save: true` bypasses a smart-save skip. `save: false` still wins, so a restore-only trust policy cannot be overridden by `force-save`. `save: true` always saves and `save-if: always` is the default automatic policy. Static expressions can still restrict this action to trusted branch writers; the example does that before applying the runtime policy and only runs pull requests from the same repository. These expressions do not restrict arbitrary AWS calls from job code. Use separate runner permissions or stacks for untrusted code; a key or restore-only input is not an IAM boundary.
